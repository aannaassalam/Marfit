...
export default class Checkout extends React.Component {
								...
								() => {
									if (this.state.cart.length > 0) {
										...
										this.state.currentUser.name.includes(" ")
											? this.setState({
													//first name
													firstName: this.state.currentUser.name.substr(0, this.state.currentUser.name.indexOf(" ")),
													//last name
	...
	render() {
		...
		for (var i = 0; i < this.state.products.length; i++) {
			...
			shipping += this.state.products[i].shippingCharge * this.state.cart[i].quantity;
		}
		total = subTotal;
						...
						<div className='left'>
							{this.state.currentUser !== "" ? null : (
								<div className='already'>
									<p>
										Already have an account? <span onClick={() => this.setState({ openLogin: true })}>Log in</span>
									</p>
								</div>
							)}
