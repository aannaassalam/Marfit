      ...
      firebase
        ...
        .then((snap) => {
          snap.forEach((doc) => {
            ...
            var outOfStock = this.state.outStockList.filter(
              (item) => item !== id
            );
        ...
        .then((snap) => {
          snap.forEach((doc) => {
            ...
            cart.forEach((item) => {
              if (
                item.id === id &&
                item.quantity < parseInt(item.max) &&
                item.quantity < parseInt(item.sizeMax)
              ) {
                item.quantity += 1;
              }
            });
            ...
            {this.state.cart.length > 0 ? (
              <button
                ...
                onClick={() =>
                  this.state.outStock
                    ? toaster.notify(
                        "Your cart contains an out of stock product, please remove it"
                      )
